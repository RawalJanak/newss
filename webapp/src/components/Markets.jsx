    ...
    <div className="tbl">
      ...
      {items.map((r, i) => (
        <div className="trow" key={i}>
          <div className="a">
            <div className="s">{r.name || r.symbol}</div>
            <div className="u">{sub && r.sector ? r.sector : r.symbol}</div>
          </div>
...
export default function Markets({ mkt, market, stamp }) {
  ...
  return (
    <>
      <div className="idxgrid">
        {(m.indices || []).map((i, k) => (
          <div className="icard" key={k}>
            <div className="n">{i.symbol}</div>
            <div className="p">{n2(i.price)}</div>
            <div className="c">{pct(i.change_pct)}</div>
          </div>
        ))}
      </div>
      <div className="read">
      ...
      <div className="note">
        Snapshot taken {stamp}, not a live feed — prices update when the digest is rebuilt.
        India and China lists come from a large-cap universe; US lists are market-wide. Data via Yahoo Finance. Not investment advice.
      </div>
    </>
  )
}
